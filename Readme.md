...
### 1. Conda Environment for Data Processing

This environment is used for binary processing and for executing scripts in the `decompilation` and `data_process` directories.

#### 1.1. Create a New Conda Environment 
```bash
conda create -n hieronym_data_process python=3.10
```

#### 1.2. Activate the Conda Environment
```bash
conda activate hieronym_data_process
```

#### 1.3. Install Required Packages
```bash
pip install -r requirements_for_data_process.txt
```

### 2. Conda Environment for Fine-Tuning and Prediction

This environment is used for summary generation, fine-tuning, prediction, and evaluation.

#### 2.1. Create a New Conda Environment 

```bash
conda create -n hieronym python=3.11
```
...
```bash
pip install -r requirements_for_train.txt
```

#### 2.4. Install vLLM for Summary Generation

This environment is only required when using Qwen3-Coder as the server.
...
Scripts are located in `scripts/decompilation/`. The IDA-based decompilation scripts are located in directories ending with `ida`. We also provide a tutorial for the Ghidra-based scripts. See the inline comments in the IDA scripts for parameter details.
...
Scripts are located in `scripts/data_processing/`. The IDA-based data processing scripts are located in directories ending with `ida`. We also provide a tutorial for the Ghidra-based scripts. See the inline comments in the IDA scripts for parameter details.
...
This step requires vLLM. First, launch the vLLM serve using Qwen3-Coder. Update `tensor-parallel-size` in `qwen3-server.yaml` (requires 4 or 8 GPUs).

```bash
vllm serve --config qwen3-server.yaml
```

Then update `VLLM_API_URL` and `VLLM_MODEL_NAME` in `summary_generation/generate_global_binary_context.py`.

```bash
python summary_generation/generate_global_binary_context.py \
...
First, launch the vLLM serve (as in step 2.4).
...
#### 5.3 Predict from scratch on a single binary

We provide the `process_single_binary` script, which performs prediction on a single binary from scratch. For the meaning of each parameter in the script, please refer to the comments within its code.
