...
MAX_CONCURRENT_REQUESTS = 64  # Control concurrent requests to prevent OOM
TIMEOUT = 600

'''
# This setting for test
VLLM_API_URL = "http://localhost:8081/v1/chat/completions"
VLLM_MODEL_NAME = "codellama-lora"
MAX_CONCURRENT_REQUESTS = 64  # Control concurrent requests to prevent OOM
TIMEOUT = 600
'''
...
def remove_all_comments(code: str) -> str:
    """Remove all comments from code while preserving strings."""
    pattern = r'''
...
async def process_function_async(
    ...
    """Process single function with LLM to generate descriptions."""
    function_name, func_body = function_item

    function_data = {function_name: {"target": function_name}}
...
def batched_stripped_functions(source_functions: Dict, training: bool) -> List[Tuple[str, Dict]]:
    """Convert source functions to batch-friendly format."""
    functions_list = []
    if training:
...
async def main_async(
    ...
    source_functions = batched_stripped_functions(source_functions, training)


    prompt_path = "./summary_generate/utils/prompt_calling_context_function"
...
def main(
    ...
    output_file = os.path.join(output_dir, f'{type}_set_with_call.json')
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='Used to generate summaries of internal functions in training or test set data.')
    parser.add_argument('-i', '--input_file', type=str, required=True,
                        help='Input path for training set file or test set file.')
    parser.add_argument('-o', '--output_dir', type=str, required=True,
                        help='Directory to save the output files.')
    parser.add_argument('--type', type=str, required=True, choices=['training', 'test', 'validation'],
                        help='Dataset type: train, test, or validation.')
