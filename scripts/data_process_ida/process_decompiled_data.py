...
def process_single_file_dataset(args):
    ...
    try:
        ...
        for function_name in unstripped_data.keys():
            ...
            function_pair = defaultdict(dict)
            ...
            if function is not None:
                ...
                if masked_code:
                    function_pair['unstripped']['code'] = remove_comments(masked_code)
                    function_pair['unstripped']['callee_external'] = function_call['callee_external']
                    function_pair['unstripped']['callee_internal'] = function_call['callee_internal']
                    function_pair['unstripped']['caller'] = function_call['caller']
            ...
            if stripped_function is not None:
                ...
                if masked_stripped_code:
                    function_pair["stripped"]['code'] = remove_comments(masked_stripped_code)
                    try:
                        function_pair["stripped"]['callee_external'] = stripped_function_call.get('callee_external', [])
                        function_pair["stripped"]['callee_internal'] = function_call.get('callee_internal', [])
                        function_pair["stripped"]['caller'] = function_call.get('caller', [])
                        function_pair["stripped_function_name"] = stripped_function["func_name"]
                    except:
                        pass

            if function_pair:
                result_dict[function_name] = function_pair
