...
class ASTProcessor:
    def __init__(self):
        ...
        self.node_map = {}
        self.max_line_filter = MAX_LINE_FILTER
...
def remove_and_renumber_nodes(ast_blocks, source_code):
    ...
    for key, blocks in ast_blocks.items():
        ...
        for block_id in range(len(blocks) - 1, -1, -1):
            block_id_map[blocks[block_id]['id']] = str(total_blocks - block_counter + 1)
            start_line, end_line = blocks[block_id]['line']
...
def format_output_line(source_code, func_blocks, block_data, block_id_map, func_type, func_name, training):
    ...
    if training:
        for key, blocks in block_data.items():
            if key in (0, 1):
                func_node = blocks[0]
                func_relations = [block_id_map[func_id] for func_id in func_node['children']]
            else:
                for block in blocks:
                    block_id = block_id_map[block['id']]
                    child_relations = [block_id_map[child_id] for child_id in block['children']]
