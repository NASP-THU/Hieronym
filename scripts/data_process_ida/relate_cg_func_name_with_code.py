...
SKIP_FUNCTIONS = ['deregister_tm_clones', 'register_tm_clones', 'do_global_dtors_aux', 'frame_dummy',
                  ...
                  '__libc_csu_fini', 'main', '__do_global_dtors_aux', '_start']
...
class FunctionInlineReplacer:
    ...
    def inline_function_calls(self, target_func_name: str) -> Dict[str, Any]:
        ...
        for symbol_type in ['unstripped', 'stripped']:
            if symbol_type not in func_data:
                continue

            callees_to_inline = []
            callers_to_inline = []

            if "callee_internal" in func_data[symbol_type]:
                callees = func_data[symbol_type]["callee_internal"]
                if isinstance(callees, list):
                    callees_to_inline.extend(callees)

            if "caller" in func_data[symbol_type]:
                callers = func_data[symbol_type]["caller"]
            ...
            for callee_name in callees_to_inline:
                ...
                if symbol_type in callee_data:
                    callee_code = callee_data[symbol_type]['code']
                    if symbol_type == 'stripped':
            ...
            for caller_name in callers_to_inline:
                ...
                if symbol_type in caller_data:
                    caller_code = caller_data[symbol_type]['code']
                    if symbol_type == 'stripped':
                        stripped_caller_name = self.functions_data[caller_name]['stripped_function_name']
                        inlined_caller_codes.append({stripped_caller_name: caller_code})
                    else:
                        inlined_caller_codes.append({caller_name: caller_code})

            if inlined_callee_codes:
                func_data[symbol_type]["callee_internal"] = inlined_callee_codes
            if inlined_caller_codes:
                func_data[symbol_type]["caller"] = inlined_caller_codes
...
def process_single_file(file_info: Tuple[str, str]) -> Tuple[str, Dict[str, Any]]:
    ...
    try:
        ...
        current_directory = os.path.dirname(file_path).split('/')[-3:]
        new_output_dir = os.path.normpath(os.path.join(output_directory, '/'.join(current_directory)))
        try:
        ...
        output_file = os.path.join(new_output_dir, binary_name)
