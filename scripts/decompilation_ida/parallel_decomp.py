...
def process_unstripped(ida_path, slot_name, binary_path):
    print(f"[*] hold {slot_name} for {binary_path}")

    script = "./decompilation_ida/decomp_for_unstripped.py"
...
def process_stripped(ida_path, slot_name, binary_path):
    print(f"[*] hold {slot_name} for {binary_path}")

    script = "./decompilation_ida/decomp_for_stripped.py"
    run_ida(ida_path, script, binary_path)
...
def main(args):
    ...
    def submit_task(binary_file_path):
        ...
        executor.submit(
            process_unstripped if args.unstripped else process_stripped,
