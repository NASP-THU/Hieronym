...
class SimplifiedCGFromDecompiler(GhidraScript):
    def run(self):
        ...
        external_functions, export_functions = self.get_external_functions()
        ...
        while function is not None:
            ...
            if func_name in export_functions:
                call_relations[func_name]['address'] = str(function.getEntryPoint())
            ...
            try:
                ...
                if decompile_results.decompileCompleted():
                    ...
                    if high_func:
                        self.extract_calls_from_high_func(high_func, func_name, call_relations, external_functions)
                else:
                    print('{} error to decompiled!'.format(func_name))
            except Exception as e:
                print("[!] Error analyzing function {}: {}".format(func_name, e))
    ...
    def extract_calls_from_high_func(self, high_func, caller_name, call_relations, external_functions):
        try:
            ...
            while pcode_iter.hasNext():
                ...
                if pcode_op.getOpcode() == PcodeOp.CALL:
                    self.process_direct_call(pcode_op, caller_name, call_relations, external_functions)
        except Exception as e:
            print("[!] Error extracting calls for {}: {}".format(caller_name, e))

    def process_direct_call(self, pcode_op, caller_name, call_relations, external_functions):
        input0 = pcode_op.getInput(0)
        if not input0:
            return
        
        call_addr = input0.getAddress()
        if not call_addr:
            return
        
        callee_func = getFunctionAt(call_addr)
        ...
        if callee_func:
            ...
            external_label = self.get_external_function_label(call_addr)
            if external_label:
                if external_label in external_functions:
